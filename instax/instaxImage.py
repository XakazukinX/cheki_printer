...
class InstaxImage:
    ...
    def convert_image(self, crop_type='middle',
...
def crop_rectangle(source, size, crop_type='top'):
    ...
    if ratio > new_ratio:
        raise Exception('[エラー] 画像サイズが600*800以下です ')
    elif ratio < new_ratio:
        img = source
        left = (int(round((img.size[0] - size[0]) / 2)))
        upper = (int(round((img.size[1] - size[1]) / 2)))
        right = left + size[0]
        bottom = upper + size[1]
        ...
        print("画像をクロップしました %s %s=>%s" % (img, prev_size, before_size))
        img.save('crop.png')
    else:
